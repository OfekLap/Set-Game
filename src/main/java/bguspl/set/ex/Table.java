...
public class Table {
    ...
    public int[] getSlotsToCheck(Player player) {
        int[] cards = new int[env.config.featureSize];
        int j = 0;
        for (int i = 0; i < slotToCard.length; i++) {
            if (playersTokens[player.id][i] == true) {
                cards[j] = i;
                j++;
            }
        }
    ...
    public boolean removeToken(int player, int slot) {
        if (slotToCard[slot] != null && playersTokens[player][slot] == true) {
