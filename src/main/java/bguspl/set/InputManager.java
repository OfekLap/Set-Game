...
/**
 * This class handles the input from the keyboard, translates it to table grid
 * slots and dispatches accordingly.
 */
        ...
        for (int player = 0; player < config.players; ++player)
            for (int i = 0; i < config.playerKeys(player).length; i++) {
                ...
                if (keyCode >= keyMap.length)
                    reallocArrays(keyCode); // enlarge the array for higher key codes
